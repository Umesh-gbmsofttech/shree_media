...
const CategoryGrid = ({ activeCategory, setActiveCategory }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-nowrap overflow-x-auto scrollbar-hide gap-2 md:gap-4 py-2 -mx-2 px-4 justify-between">
        { categories.map((cat) => (
          <button
            key={ cat.name }
            onClick={ () => handleCategoryClick(cat.name) }
            className={ `flex flex-col items-center group transition-all duration-500 ease-out flex-shrink-0 ${activeCategory === cat.name ? '' : 'opacity-100'
              }` }
          >
